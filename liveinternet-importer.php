...
//ф-я вызывается на первом шаге работы плагина
        function greet() {
            ...
            echo '<p>' . __('In this case new permalinks for your post will be generated using your permalink settings') . '</p>';
            ...
            echo '<p>' . __('If you want save your posts links like http://your blog url/post210378848/ choose this button') . '</p>';
            echo '<p>' . __('You need set up your permalink settings like %postname% before upload.') . '</p>';
            wp_import_upload_form("admin.php?import=liru&amp;step=2");
            echo '</div>';
        }

//тут происходит вся работа и все самое интересное
        function import_posts($perm = false) {
            ...
            foreach ($posts as $post) {
                ...
                //статус - опубликовано
                $post_status = 'publish';
                //теперь проверим нужно ли сохранить ссылки
                if ($perm) {
                    preg_match('|<link>(.*?)</link>|is', $post, $post_name);
                    $post_name = str_replace(array('<![CDATA[', '/]]>'), '', trim($post_name[1]));
                    preg_match('|post(.*)|is', $post_name, $post_name);                  
                ...
                if ($post_id = post_exists($post_title, $post_content, $post_date)) {
                    ...
                    if ($perm) {
                        echo '<br> Your new post link is ' . $post_name;
                        $postdata = compact('post_author', 'post_date', 'post_content', 'post_name', 'post_title', 'post_status', 'tags_input');
                    } else {
                ...
                //а вот тут будет еще перенос комментариев
                preg_match('|<slash:comments>(.*?)</slash:comments>|is', $post, $comm_count);                
                $comm_count = (int) $comm_count[1];
                if ($comm_count > 0) {
                    echo '<br> You have ' . $comm_count . ' comments to this post. 
                        Comments Import will be added in next version! ';
                }
                //preg_match('|<wfw:commentRss>(.*?)</wfw:commentRss>|is', $post, $comm_rss);
                //$comm_rss = str_replace(array('<![CDATA[', ']]>'), '', trim($comm_rss[1]));
            }
        }

//ф-я управляет ходом импорта, этакий контроллер
        function import($perm = false) {
